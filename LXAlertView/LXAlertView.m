...
-(instancetype)initWithTitle:(NSString *)title message:(NSString *)message cancelBtnTitle:(NSString *)cancelTitle otherBtnTitle:(NSString *)otherBtnTitle clickIndexBlock:(LXAlertClickIndexBlock)block{
    if(self=[super init]){
        ...
        _messageTF.frame=CGRectMake(messageLabSpace, _titleLab.frame.size.height+_titleLab.frame.origin.y+10+10, AlertView_W-messageLabSpace*2, endMessageLabH-20);
        _messageTF.borderStyle= UITextBorderStyleRoundedRect;
        _messageTF.placeholder = @"新建文件夹";
        ...
        [_alertView addSubview:_messageLab];
         [_alertView addSubview:_messageTF];
        if (cancelTitle) {
